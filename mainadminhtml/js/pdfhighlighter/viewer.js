...
if (typeof window !== 'undefined' && window['pdfjs-dist/build/pdf']) {
  ...
  pdfjsLib = require('js/pdfhighlighter/pdf.js');
}
...
function configure(PDFJS) {
  PDFJS.imageResourcesPath = 'js/pdfhighlighter/images/';
  PDFJS.workerSrc = 'js/pdfhighlighter/pdf.worker.js';
...
var THUMBNAIL_CANVAS_BORDER_WIDTH = 0;
var THUMBNAIL_WIDTH = 10;
var TempImageFactory = function TempImageFactoryClosure() {
  ...
  var j = 0;
  var l=0;
  var pagesno = website('.getpdfpagesno').val();
  var res = pagesno.split("=");
  for(var i=0;i<res.length;i++){

       if(i%2==0){

          arr_pagesname.push(res[i]);
            
       }else{
        arr_pagesno.push(res[i]);
       }
  }
...
var PDFThumbnailView = function () {
  function PDFThumbnailView(_ref) {
    ...
    this.l10n.get('thumb_page_title', { page: id }, 'Page {{page}}').then(function (msg) {
      //console.log(arr_pagesname[0]);
            
     if(arr_pagesno[j]==id){
         anchor.title = arr_pagesname[j]; 
         j++;        
      }
    ...
    if(arr_pagesno[k]==id){
      ...
      var links = arr_pagesname[l];       
     var newContent = document.createTextNode(links);
...
var DEFAULT_URL = website('.getpdfpath').val();
//var DEFAULT_URL = website('.getpdfpath').val();
