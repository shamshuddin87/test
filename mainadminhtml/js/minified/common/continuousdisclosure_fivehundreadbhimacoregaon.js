...
{for(var i=0;i<response.data.length;i++)
{var j=i+1;var sentdate=response.data[i]['sent_date']?response.data[i]['sent_date']:"";addhtmlnxt+='<tr class="counter">';addhtmlnxt+='<td width="5%">'+j+'</td>';addhtmlnxt+='<td width="20%">'+response.data[i]['date_added']+'</td>';addhtmlnxt+='<td width="15%"><i class="fa fa-paper-plane sendpdf" reqid="'+response.data[i]["srno"]+'"></i></td>';addhtmlnxt+='<td width="15%">'+response.data[i]["annualyear"]+'</i></td>';addhtmlnxt+='<td width="15%">'+sentdate+'</td>';if(response.data[i]['send_status']==0)
{addhtmlnxt+='<td width="25%"><a  href="'+response.data[i]["pdfpath"]+'"  class="downlodthfle" style="color:black;"><span class="glyphicon glyphicon-download-alt floatleft"></span></a> <span class="glyphicon glyphicon-trash delfile" delid="'+response.data[i]["srno"]+'"></span> <a href = " continuousdisclosure/editcont_disclsr?id='+response.data[i]['uniqueid']+'" <span class="glyphicon glyphicon-edit editfile" editid="'+response.data[i]["uniqueid"]+'"></span></a></td>';}
else
