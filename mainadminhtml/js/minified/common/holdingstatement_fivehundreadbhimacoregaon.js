...
{var getdate=website(this).val();var getid=website(this).closest('form').attr('id');});});website('#insertholdingstatement').ajaxForm({dataType:"json",beforeSend:function()
...
{website.ajax({url:'holdingstatement/fetchholdingstatement',method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{var addhtmlnxt='';for(var i=0;i<response.resdta.length;i++)
{var date=response.resdta[i].date_modified;date=date.split(' ')[0];dtfrmt=date.split("-");dtfrmtspace=date.split(" ");ddmmyy=dtfrmtspace[0];dtfrmt=dtfrmtspace[0].split("-");ddmmyy=dtfrmt[2]+'-'+dtfrmt[1]+'-'+dtfrmt[0];addhtmlnxt+='<tr class="counter" aprvllistid="'+response.resdta[i].id+'" >';addhtmlnxt+='<td width="25%">'+response.resdta[i].month+'</td>';addhtmlnxt+='<td width="25%">'+response.resdta[i].year+'</td>';addhtmlnxt+='<td width="25%"><a href="'+response.resdta[i].filepath+'" download>&nbsp;<i class="fa fa-download" id="uploadattached1" aria-hidden="true"></i></a></td>';addhtmlnxt+='<td width="25%">'+ddmmyy+'</td>';addhtmlnxt+='<td width="25%" ><i class="fa fa-trash-o faicon floatleft deleterestrictedcmp" title="Delete entry" aprvllistid="'+response.resdta[i].id+'" ></i></td>';addhtmlnxt+='</tr>';}
