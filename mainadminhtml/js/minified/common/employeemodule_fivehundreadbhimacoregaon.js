...
{var url=new URL(window.location.href);var action=url.searchParams.has("from");if(action)
...
{new PNotify({title:'Alert',text:'Please Enter Holding in Shares',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}
else if(!adrsholdng)
{new PNotify({title:'Alert',text:'Please Enter Holding in ADRs',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}}
else if(id=="relsub")
...
{new PNotify({title:'Alert',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}},complete:function(response)
{website('.preloder_wraper').fadeOut();},error:function()
{}});getpersonalinfo();function getpersonalinfo(){website.ajax({url:'employeemodule/getmydetails',method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{var myhtml='<tr>';if(response.logged==true){obj=response.data;var fname=obj['fname']?obj['fname']:'';var pan=obj['pan']?obj['pan']:'';var dob=obj['dob']?obj['dob']:'';var aadhar=obj['aadhar']?obj['aadhar']:'';var education=obj['education']?obj['education']:'';var institute=obj['institute']?obj['institute']:'';var filepath=obj['filepath']?obj['filepath']:'';var dpdate=obj['dpdate']?obj['dpdate']:'';var mobno=obj['mobileno']?obj['mobileno']:'';var id=response.data.id?response.data.id:'';myhtml+='<td>1</td>';myhtml+='<td>'+pan+'</td>';myhtml+='<td>'+aadhar+'</td>';myhtml+='<td>'+dpdate+'</td>';myhtml+='<td>'+dob+'</td>';myhtml+='<td>'+education+'</td>';myhtml+='<td>'+institute+'</td>';myhtml+='<td>'+mobno+'</td>';if(filepath)
{myhtml+='<td><a href="'+filepath+'" download>&nbsp;<i class="fa fa-download" id="uploadattached1" aria-hidden="true"></i></a></td>';}
else
{myhtml+='<td></td>';}
myhtml+='<td><i class="fa fa-eye faicon floatleft viewdetail" title="View entry" id="editdetail" personalid="'+response.data.id+'"></i></td>';if(response.data.send_status==1)
{myhtml+='<td>---</td>';}
else
...
else{myhtml+='<td colspan="9" style="text-align:center;">Data Not Found..!!</td>'}
myhtml+='</tr>';website('.perdetail').html(myhtml);},complete:function(response)
{},error:function(jqXHR,textStatus,errorThrown)
{}});website('body').on('click','.del',function(e){delid=website(this).attr('delid');website('#deleteid').val(delid);website('#delmod').modal('show');});website('body').on('click','#delinfo',function(e){var delid=website('#deleteid').val();website.ajax({url:'employeemodule/delmydetails',data:{delid:delid},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{website('#delmod').modal('hide');if(response.logged==true)
{getpersonalinfo();new PNotify({title:'Alert',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}
else
{new PNotify({title:'Alert',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}}});});website('body').on('click','.editcmp',function(e)
{editid=website(this).attr('editid');website.ajax({url:'employeemodule/getmydetails',method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{var myhtml='<tr>';if(response.logged==true)
{website('#mydataedit').modal('show');var id=response.data.id?response.data.id:'';var mobileno=response.data.mobileno?response.data.mobileno:'';var upmyhtml='';var clid='';var pan=obj['pan']?obj['pan']:'';var dob=obj['dob']?obj['dob']:'';var sex=obj['sex']?obj['sex']:'';var aadhar=obj['aadhar']?obj['aadhar']:'';var education=obj['education']?obj['education']:'';var institute=obj['institute']?obj['institute']:'';var address=obj['address']?obj['address']:'';var filepath=obj['filepath']?obj['filepath']:'';website('#mydataedit #upmobno').val(mobileno)
website('#mydataedit #pan').val(pan);website('#mydataedit #aadhar').val(aadhar);website('#mydataedit #dob').val(dob);website('#mydataedit #address').val(address);website('#mydataedit #eduqulfcn').val(education);website('#mydataedit #institute').val(institute);website('#mydataedit #reqid').val(id);website('#mydataedit #upmobileappend').remove();website('#mydataedit #addmobileonmd').html(upmyhtml);website('#mydataedit #filepath').val(filepath);jQuery("input[value='"+sex+"']").attr('checked',true);}
else
...
{},error:function(jqXHR,textStatus,errorThrown)
{}});});}
website('body').on('click','.addrelinfo',function(e){var myform=website(this).attr('formno');var newno=parseInt(myform)+1;myhtml='<form method="post" id="getdata_'+newno+'" class="chklength"  enctype="multipart/form-data"><div class="input-group col-md-12 col-xs-12 col-sm-12 remdiv_'+newno+'">'+'<div class="col-md-3">'+'<label>Full name</label>'+'<input  class="form-control " placeholder="full name" id="'+newno+'_fname"  name="'+newno+'_fname" type="text" />'+'</div>'+'<div class="col-md-3">'+'<label>Pan</label>'+'<input  class="form-control panval"  placeholder="pan" id="'+newno+'_pan" name="'+newno+'_pan" type="text" />'+'</div>'+'<div class="col-md-3">'+'<label>Aadhar</label>'+'<input  class="form-control aadhar"  placeholder="aadhar" id="'+newno+'_aadhar" name="'+newno+'_aadhar" type="text" />'+'</div>'+'<div class="col-md-3">'+'<label>Age</label>'+'<input  class="form-control " placeholder="age" id="'+newno+'_age" name="'+newno+'_age" type="text" />'+'</div>'+'<div class="col-md-3">'+'<label for="sex">Sex:</label>'+'<input type="radio" id="'+newno+'_sex" name="'+newno+'_sex" value="Male" checked/>Male'+'<input type="radio" id="'+newno+'_sex" name="'+newno+'_sex"  value="Female"/>Female '+'<div class="col -md-3">  </div>'+'<lable>Date of birth</lable>'+'<input type="text" name="'+newno+'_dob" id="'+newno+'_dob" class="bootdatepick" placeholder="dob">'+'</div>'+'<div class="col-md-3">'+'<lable>Relationship</lable>'+'<input  class="form-control " placeholder="relationship" id="'+newno+'_relationship"  name="'+newno+'_relationship" type="text" />'+'</div>'+'<div class="col-md-3">'+'<label >Address</label>'+'<textarea class="form-control " placeholder="address" id="'+newno+'_addr"  name="'+newno+'_address" type="text"></textarea>'+'</div>'+'</form>';var buttonhtml='<div class="valida"><span class="glyphicon glyphicon-plus addrelinfo" style="color:green;" formno="'+newno+'"></span><span class="glyphicon glyphicon-minus deleterelinfo" style="color:red;" formno="'+newno+'"></span></div>';website('.addrelinfo').remove();website('.deleterelinfo').remove();website('.appndnew').append(myhtml);website('.appndnew').append(buttonhtml);datepicker();aadharvalidation();});website('body').on('click','.deleterelinfo',function(e){var myform=website(this).attr('formno');var newno=myform-1;if(newno>=1){website('.addrelinfo').remove();website('.deleterelinfo').remove();var buttonhtml='<span class="glyphicon glyphicon-plus addrelinfo" style="color:green;" formno="'+newno+'"></span><span class="glyphicon glyphicon-minus deleterelinfo" style="color:red;" formno="'+newno+'"></span>';website(".remdiv_"+myform).remove();website('.appndnew').append(buttonhtml);}});getrelationdata();function getrelationdata(){website.ajax({url:'employeemodule/getrelativedata',method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{var j=1;var myhtml='';for(var i=0;i<response.data.length;i++)
{var education=response.data[i].education?response.data[i].education:'';var sharehldng=response.data[i].sharehldng?response.data[i].sharehldng:'';var adrshldng=response.data[i].adrshldng?response.data[i].adrshldng:'';myhtml+='<tr>';myhtml+='<td>'+j+'</td>';myhtml+='<td>'+response.data[i].relationshipname+'</td>';myhtml+='<td>'+response.data[i].name+'</td>';myhtml+='<td>'+response.data[i].pan+'</td>';myhtml+='<td>'+response.data[i].aadhar+'</td>';myhtml+='<td>'+response.data[i].mobile+'</td>';myhtml+='<td>'+response.data[i].dob+'</td>';myhtml+='<td>'+education+'</td>';if(response.data[i].filepath)
...
{myhtml+='<tr>';myhtml+='<td colspan="11" style="text-align:center;">Data Not Found..!!</td>';myhtml+='</tr>';}
...
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged==true){new PNotify({title:'Alert',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});getrelationdata();}
...
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged==true){document.getElementById("relsub").disabled=true;website(".valida").css("display","none");new PNotify({title:'Alert',text:"Aadhar No Already Exist You Can Not Insert Data",type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}}});}
else
{document.getElementById("relsub").disabled=false;website(".valida").css("display","block");}});}
panvalidation();function panvalidation()
{website(".panval").on("keyup",function(){var panno=website(this).val();len=panno.toString().length;if(len>=5&&panno!='')
{website.ajax({url:'employeemodule/panvalidation',data:{panno:panno},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged==true)
...
{var id=website(this).attr('personalid');var baseHref=getbaseurl();window.location.href=baseHref+'employeemodule/viewpastemployer';});website('#upmfrmod').click(function(e){var mfreditid=website('#mfreditid').val();var mfrname=website('#mfrnameup').val();var panup=website('#adharpanup').val();var addressup=website('#materialaddressup').val();var mfrrelation=website('#mfrrelationup').val();var transaction=website('#mfrtransactionup').val();var clientid=website('#mfrclientidup').val();var mobile=website('#mfrmobileup').val();website.ajax({url:'employeemodule/updatemfr',data:{mobile:mobile,mfrname:mfrname,mfrrelation:mfrrelation,mfreditid:mfreditid,panup:panup,addressup:addressup,transaction:transaction,clientid:clientid},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
...
{if(response.logged===true)
{var j=1;var myhtml='';for(var i=0;i<response.data.length;i++)
{myhtml+='<tr>';myhtml+='<td>'+j+'</td>';myhtml+='<td>'+response.data[i].related_party+'</td>';myhtml+='<td>'+response.data[i].pan+'</td>';myhtml+='<td>'+response.data[i].relationship+'</td>';myhtml+='<td>'+response.data[i].mobile+'</td>';myhtml+='<td>'+response.data[i].address+'</td>';myhtml+='<td><i class="fa fa-edit mymfredit"  mfredit="'+response.data[i].id+'" style="font-size:20px;"></i><i class="fa fa-trash delmfr"  mfrdel="'+response.data[i].id+'" style="font-size:20px;"></i></td>';myhtml+='</tr>';j++;}}
else
{myhtml+='<tr>';myhtml+='<td colspan="6" style="text-align:center;">Data Not Found..!!</td>';myhtml+='</tr>';}
...
{},error:function(jqXHR,textStatus,errorThrown)
{}});});website('body').on('click','#delbtnmfr',function(e){delid=website('#mfrdelid').val();website.ajax({url:'employeemodule/deletemfr',data:{delid:delid},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged==true){new PNotify({title:'Alert',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});getmfrdata();}
else{new PNotify({title:'Alert',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}},complete:function(response)
{website('#mfrdelmodal').modal('hide');},error:function(jqXHR,textStatus,errorThrown)
{}});});website('body').on('click','#trdeintimatn',function()
{website('#tradeintimationmodel').modal('show');});var timer=0;function mySearch(){var getvalue=website('.header-search-input').val();doSearch(getvalue);}
...
{}});}}
website('body').on('click','.validatorsid',function(e){var compid=website(this).attr('id');var compname=website(this).attr('company_name');website('#tradeintimationmodel #search-box').val(compname);website('#search-box').attr('compid',compid);website('#search-box').attr('compname',compname);website('#tradeintimationmodel #compid').val(compid);website('#live-search-header-wrapper').fadeOut();website('#tradeintimationmodel #validators').val(compname);website('#validators').attr('compid',compid);website('#validators').attr('compnyname',compname);});website('body').on('click','#Mymodaledit .validatorsid',function(e){var compid=website(this).attr('id');var compname=website(this).attr('company_name');website('#updatetrdintimtn #search-box').val(compname);website('#Mymodaledit #search-box').attr('compid',compid);website('#Mymodaledit #search-box').attr('compname',compname);website('#Mymodaledit #live-search-header-wrapper').fadeOut();website('#updatetrdintimtn #validators').val(compname);website('#updatetrdintimtn #compid').val(compid);website('#Mymodaledit #validators').attr('compid',compid);website('#Mymodaledit #validators').attr('compnyname',compname);});website(function(){});website('#trnstype').on('change',function(){var value=website(this).val();if(value==5)
{website('#dateoftrans').hide();}
else
{website('#dateoftrans').show();}});website('#inserttrdintimtn').ajaxForm({dataType:"json",beforeSend:function()
{website('.preloder_wraper').fadeIn();},uploadProgress:function(event,position,total,percentComplete)
{website('.preloder_wraper').fadeIn();},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{new PNotify({title:'Record Added Successfully',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});fetchtradeeintimtn();}else{new PNotify({title:'Record Not Added',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}},complete:function(response)
{website('#tradeintimationmodel').modal('hide');website('.preloder_wraper').fadeOut();},error:function(jqXHR,textStatus,errorThrown)
{}});website('#updatetrdintimtn').ajaxForm({dataType:"json",beforeSend:function()
{website('.preloder_wraper').fadeIn();},uploadProgress:function(event,position,total,percentComplete)
{website('.preloder_wraper').fadeIn();},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{new PNotify({title:'Record Updates Successfully',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});fetchtradeeintimtn();}else{new PNotify({title:'Record Not Updates',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}},complete:function(response)
{website('#tradeintimationmodel').modal('hide');website('.preloder_wraper').fadeOut();},error:function(jqXHR,textStatus,errorThrown)
{}});fetchtradeeintimtn();function fetchtradeeintimtn()
{website.ajax({url:'employeemodule/fetchtradeeintimtn',method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{var j=1;var myhtml='';for(var i=0;i<response.data.length;i++)
{var transctndate=response.data[i].transaction_date?response.data[i].transaction_date:'';myhtml+='<tr>';myhtml+='<td>'+j+'</td>';myhtml+='<td>'+response.data[i].related_party+'</td>';myhtml+='<td>'+response.data[i].security_type+'</td>';myhtml+='<td>'+response.data[i].company_name+'</td>';myhtml+='<td>'+response.data[i].noofshares+'</td>';myhtml+='<td>'+transctndate+'</td>';myhtml+='<td>'+response.data[i].transaction+'</td>';myhtml+='<td><i class="fa fa-edit trdintmtnedit"  mfredit="'+response.data[i].id+'" style="font-size:20px;"></i><i class="fa fa-trash deltrdintmtn"  trdintmtndel="'+response.data[i].id+'" style="font-size:20px;"></i></td>';myhtml+='</tr>';j++;}}
else
{myhtml+='<tr>';myhtml+='<td colspan="8" style="text-align:center;">Data Not Found..!!</td>';myhtml+='</tr>';}
website('.trdeintimatndetail').html(myhtml);},complete:function(response)
{},error:function(jqXHR,textStatus,errorThrown)
{}});}
website('body').on('click','.trdintmtnedit',function()
{var editid=website(this).attr('mfredit');website.ajax({url:'employeemodule/fetchtradeeintimtnedit',data:{editid:editid},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged==true)
{website('#Mymodaledit #validators').val(response.data[0].company_name);website('#Mymodaledit #reltedprty').val(response.data[0].related_party);website('#Mymodaledit #secutype').val(response.data[0].security_type);website('#Mymodaledit #trnstype').val(response.data[0].transctn_type);website('#Mymodaledit #shres').val(response.data[0].noofshares);if(response.data[0].transctn_type==5)
{website('#Mymodaledit #dateoftrans').hide();}
else
{website('#Mymodaledit #transdate').val(response.data[0].transaction_date);}
website('#Mymodaledit #trdeditid').val(editid);website('#Mymodaledit #compid').val(response.data[0].cmp_id);website('#Mymodaledit').modal('show');}
else
{}},complete:function(response)
{website('#mfrdelmodal').modal('hide');},error:function(jqXHR,textStatus,errorThrown)
{}});})
website('body').on('click','.deltrdintmtn',function(){var id=website(this).attr('trdintmtndel');website('#myModalyesno').modal('show');website('#myModalyesno .yesconfirm').attr('trdintmtndel',id);});website('body').on('click','.yesconfirm',function(){var id=website(this).attr('trdintmtndel');var formdata={id:id};website.ajax({url:'employeemodule/trdintimtndelete',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{fetchtradeeintimtn();new PNotify({title:'Record Deleted Successfully',text:'Record Deleted Successfully',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}
else
...
{website('#myModalyesno').modal('hide');website('#myModalyesno .mainprogressbarforall').fadeOut();},error:function()
{}});});website('#updatetrdintimtn').ajaxForm({dataType:"json",beforeSend:function()
{website('.preloder_wraper').fadeIn();},uploadProgress:function(event,position,total,percentComplete)
{website('.preloder_wraper').fadeIn();},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{new PNotify({title:'Record Updated Successfully',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});fetchtradeeintimtn();}else{new PNotify({title:'Record Not Updated',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}},complete:function(response)
{website('#Mymodaledit').modal('hide');website('.preloder_wraper').fadeOut();},error:function(jqXHR,textStatus,errorThrown)
{}});website('#Mymodaledit #trnstype').on('change',function(){var value=website(this).val();if(value==5)
{website('#Mymodaledit #dateoftrans').hide();}
else
{website('#Mymodaledit #dateoftrans').show();}});website('#insertpastemp').ajaxForm({dataType:"json",beforeSend:function()
{website('.preloder_wraper').fadeIn();},uploadProgress:function(event,position,total,percentComplete)
{website('.preloder_wraper').fadeIn();},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{new PNotify({title:'Record Added Successfully',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});window.location.reload();}else{new PNotify({title:'Record Not Added',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}},complete:function(response)
{website('.preloder_wraper').fadeOut();},error:function(jqXHR,textStatus,errorThrown)
{}});website('body').on('click','.add_button',function(e)
{var getno=website('#pastemp').val();if(getno<=5)
{website("#pstbtn").css("display","block");var htmlele='<div class="box_css">';for(var i=0;i<getno;i++)
{htmlele+='<section class="col col-md-6 col-xs-6">'+'<div class="input">'+'<label class="control-label">Name of employer*</label>'+'<input type="text" id="empname_'+i+'" name="empname" class="form_fields form-control col-md-7 col-xs-12 empnm" required>'+'</div>'+'</section>'+'<section class="col col-md-6 col-xs-6">'+'<div class="input">'+'<label class="control-label">Designation Served*</label>'+'<input type="text" id="designtn_'+i+'" name="designtn" class="form_fields form-control col-md-7 col-xs-12 desig" required>'+'</div>'+'</section>'+'<section class="col col-md-6 col-xs-6">'+'<div class="input">'+'<label class="control-label">Start Date of Employment*</label>'+'<input type="text" name="strtdte" id="strtdte_'+i+'" class="form-control bootdatepick sde" required readonly>'+'</div>'+'</section>'+'<section class="col col-md-6 col-xs-6">'+'<div class="input">'+'<label class="control-label">End Date of employent*</label>'+'<input type="text" name="enddte" id="enddte_'+i+'" class="form-control bootdatepick ede" required readonly>'+'</div>'+'</section>';}
htmlele+='<section class="col col-md-12 col-xs-12 company_asses" id="pstbtn">'
htmlele+='<input type="button" value="Submit" class="btn btn-primary contractexcelbtn" id="pastbtnsub">'+'</section></div>';website('#addnoofforms').html(htmlele);datepicker();}
else
{new PNotify({title:'Please Enter value Less Than Five',text:'Please Enter value Less Than Five',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}});website("body").on("click",".upremovemob",function(e)
{var id=this.id;var mobileno=website('#'+id).attr("upmobno");var gtapmb=website("#mydataedit #upmobileno").val();gtapmb=gtapmb.split(",");var position=gtapmb.indexOf(mobileno);gtapmb.splice(position,1);var arr=gtapmb.toString()
console.log(arr);website('#mydataedit #upmobileno').val(arr);website('#'+id).remove();});website(document).ready(function()
{var i=0;website("body").on("click","#mydataedit  #upaddmobile",function(e){var mobileno=website('#mydataedit  #upmobno').val();var gtapmb=website("#mydataedit  #upmobileno").val();if(gtapmb==''&&mobileno!=''&&mobileno.length>=10)
{str=mobileno.toString();clid="my_id"+i;var myhtml="<ul class='upremovemob' id='"+clid+"' upmobno='"+mobileno+"'><li>"+mobileno+"<span class='close' >&times;</span></li></ul>";website('#mydataedit  #addmobileonmd').append(myhtml);website("#mydataedit  #upmobileno").val(str);new PNotify({title:'Mobile No Added',text:'Mobile No Added',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});i++;}
else if(mobileno!=''&&mobileno.length>=10)
{var myarr=gtapmb.split(",");myarr.push(mobileno);clid="my_id"+i;var myhtml="<ul class='upremovemob' id='"+clid+"' upmobno='"+mobileno+"'><li>"+mobileno+"<span class='close' >&times;</span></li></ul>";website('#mydataedit  #addmobileonmd').append(myhtml);str=myarr.toString();website("#mydataedit  #upmobileno").val(str);new PNotify({title:'Mobile No Added',text:'Mobile No Added',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});i++;}
else
{new PNotify({title:'Invalid Mobile No',text:'Invalid Mobile No',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}});});website("body").on("click",".removemob",function(e)
{var id=this.id;var mobileno=website('#'+id).attr("mobno");var gtapmb=website("#mobileno").val();gtapmb=gtapmb.split(",");var position=gtapmb.indexOf(mobileno);gtapmb.splice(position,1);var arr=gtapmb.toString()
website('#mobileno').val(arr);website('#'+id).remove();});website(document).ready(function()
{var i=0;website("body").on("click","#addmobile",function(e){var mobileno=website('#mobno').val();var gtapmb=website("#mobileno").val();var flag=0;if(mobileno.length>=10)
{if(gtapmb==''&&mobileno!='')
{str=mobileno.toString();clid="my_id"+i;var myhtml="<ul class='removemob' id='"+clid+"' mobno='"+mobileno+"'><li>"+mobileno+"<span class='close' >&times;</span></li></ul>";website('#mobileappend').append(myhtml);website("#mobileno").val(str);new PNotify({title:'Mobile No Added',text:'Mobile No Added',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});i++;}
else if(mobileno!='')
{var myarr=gtapmb.split(",");myarr.push(mobileno);clid="my_id"+i;var myhtml="<ul class='removemob' id='"+clid+"' mobno='"+mobileno+"'><li>"+mobileno+"<span class='close' >&times;</span></li></ul>";website('#mobileappend').append(myhtml);str=myarr.toString();website("#mobileno").val(str);new PNotify({title:'Mobile No Added',text:'Mobile No Added',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});i++;}}
else
{new PNotify({title:'Please Enter Valid Mobile No',text:'Please Enter Valid Mobile No',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}});});website("body").on("click","#pastbtnsub",function(e){var nooffield=website('.empnm').length;var myarr=[];for(var i=0;i<nooffield;i++)
{var empname=website('#empname_'+i).val();var designtn=website('#designtn_'+i).val();var strtdte=website('#strtdte_'+i).val();var enddte=website('#enddte_'+i).val();if(empname!=''&&designtn!=''&&strtdte!=''&&enddte!='')
{var obj={empname:empname,designtn:designtn,strtdte:strtdte,enddte:enddte};myarr.push(obj);}
else
...
{website('.preloder_wraper').fadeIn();},uploadProgress:function(event,position,total,percentComplete)
{website('.preloder_wraper').fadeIn();},success:function(response,textStatus,jqXHR)
{if(response.logged==true){new PNotify({title:'Alert',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});website('#sendappp').modal('hide');location.reload();}
else{new PNotify({title:'Alert',text:response.message,type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}},complete:function(response)
{website('.preloder_wraper').fadeOut();},error:function(jqXHR,textStatus,errorThrown)
{}});});getpastempdata();function getpastempdata()
...
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{var htmlele='';for(var i=0;i<response.resdta.length;i++)
{response.resdta
var emp_name=response.resdta[i].emp_name?response.resdta[i].emp_name:'';var emp_desigtn=response.resdta[i].emp_desigtn?response.resdta[i].emp_desigtn:'';var startdate=response.resdta[i].startdate?response.resdta[i].startdate:'';var enddate=response.resdta[i].enddate?response.resdta[i].enddate:'';htmlele+='<div class="box_css">'+'<div class="col-md-12 text-right"><i class="fa fa-trash deletepastemp" id ="'+response.resdta[i].id+'"  aria-hidden="true" onclick="deleteemp(this.id);"></i></div>'+'<section class="col col-md-6 col-xs-6">'+'<div class="input">'+'<label class="control-label">Name of employer*</label>'+'<input type="text" id="empid_'+i+'" name="empid" class="form_fields form-control col-md-7 col-xs-12 empnm"  value= "'+response.resdta[i].id+'"  empid ="'+response.resdta[i].id+'"  style = "display:none;">'+'<input type="text" id="empname_'+i+'" name="empname" class="form_fields form-control col-md-7 col-xs-12 empnm" value= "'+emp_name+'" empname= "'+emp_name+'" required>'+'</div>'+'</section>'+'<section class="col col-md-6 col-xs-6">'+'<div class="input">'+'<label class="control-label">Designation Served*</label>'+'<input type="text" id="designtn_'+i+'" name="designtn" class="form_fields form-control col-md-7 col-xs-12 desig" value= "'+emp_desigtn+'" emp_desigtn= "'+emp_desigtn+'" required>'+'</div>'+'</section>'+'<section class="col col-md-6 col-xs-6">'+'<div class="input">'+'<label class="control-label">Start Date of Employment*</label>'+'<input type="text" name="strtdte" id="strtdte_'+i+'" class="form-control bootdatepick sde" value= "'+startdate+'"  startdate= "'+startdate+'"  required readonly>'+'</div>'+'</section>'+'<section class="col col-md-6 col-xs-6">'+'<div class="input">'+'<label class="control-label">End Date of employent*</label>'+'<input type="text" name="enddte" id="enddte_'+i+'" class="form-control bootdatepick ede" value= "'+enddate+'"  enddate= "'+enddate+'" required readonly>'+'</div>'+'</section></div>';}
htmlele+='<section class="col col-md-12 col-xs-12 company_asses" id="pstbtn">'
htmlele+='<input type="button" value="Update" class="btn btn-primary contractexcelbtn" id="pastupdate">'+'</section>';website('#addnoofforms').html(htmlele);datepicker();}
else
