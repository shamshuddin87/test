...
{var date=response.resdta[i].date_modified;date=date.split(' ')[0];dtfrmt=date.split("-");dtfrmtspace=date.split(" ");ddmmyy=dtfrmtspace[0];dtfrmt=dtfrmtspace[0].split("-");ddmmyy=dtfrmt[2]+'-'+dtfrmt[1]+'-'+dtfrmt[0];addhtmlnxt+='<tr class="counter" aprvllistid="'+response.resdta[i].id+'" >';addhtmlnxt+='<td width="25%">'+response.resdta[i].company_name+'</td>';addhtmlnxt+='<td width="25%">'+response.resdta[i].restriction_from+'</td>';addhtmlnxt+='<td width="25%">'+response.resdta[i].restriction_to+'</td>';addhtmlnxt+='<td width="25%">'+ddmmyy+'</td>';if(response.user_group_id=='2')
...
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{var appendhtml='';website("#Mymodaledit #validators").val(response.data['0'].company_name);website("#Mymodaledit #compid").val(response.data['0'].listed_company);website("#Mymodaledit #perdresfrom").val(response.data['0'].restriction_from);if(response.data['0'].restriction_to)
{website("#Mymodaledit #perdresto").val(response.data['0'].restriction_to);website("#Mymodaledit #perpetuity").prop("checked",false);}
else
