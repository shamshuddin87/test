...
{var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var formdata={noofrows:noofrows,pagenum:pagenum};website.ajax({url:'sebi/fetchformddataforaprvl',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
...
addhtmlnxt+='<tr class="counter" aprvllistid="'+response.resdta[i].id+'" >';dtfrmt=response.resdta[i].date_added.split("-");dtfrmtspace=response.resdta[i].date_added.split(" ");ddmmyy=dtfrmtspace[0];dtfrmt=dtfrmtspace[0].split("-");ddmmyy=dtfrmt[2]+'-'+dtfrmt[1]+'-'+dtfrmt[0];times=dtfrmtspace[1];addhtmlnxt+='<td width="15%" >'+ddmmyy+'  '+times+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].fullname+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].pan+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].cin+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].address+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].mobile+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].cmpcnct+'</td>';if(response.resdta[i].draft)
...
{}});}
website('body').on('click','#apprvrqstformd',function(){var id=website(this).attr('formdid');var pdfurl=website(this).attr('pdfurl');var formdata={id:id,pdfurl:pdfurl};website.ajax({url:'sebi/apprvrqstformd',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
