...
{var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var formdata={noofrows:noofrows,pagenum:pagenum};website.ajax({url:'sebi/fetchformdtransdata',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
...
{}});}
website('body').on('click','#formdsend',function(){var ids=[];website.each(website("input[name='check']:checked"),function(){ids.push(website(this).val());});var trdeid=website(this).attr('trdeid');var apprvid=website('.approverid').val();var cin=website('.cin').val();var category=website('.category').val();if(ids.length==0)
{new PNotify({title:'Alert!!',text:'Please Select Atleast One Record',type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}
else
{var formdata={ids:ids,apprvid:apprvid,cin:cin,category:category,trdeid:trdeid};website.ajax({url:'sebi/insertformd',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{var baseHref=getbaseurl();window.location.href=baseHref+'sebi/formd';}},complete:function(response)
{},error:function(jqXHR,textStatus,errorThrown)
{}});}});website('body').on('click','#formdprevious',function(){var baseHref=getbaseurl();window.location.href=baseHref+'sebi/viewtransformd';});function numberalphOnly()
