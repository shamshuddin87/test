...
{var rscrntpg=website('.gotobtn').val();website('.panel.panel-white #pagenum').val(rscrntpg);getdataonload();})
function getdataonload()
{var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var formdata={noofrows:noofrows,pagenum:pagenum};website.ajax({url:'sebi/fetchformbdataforaprvl',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
...
addhtmlnxt+='<tr class="counter" aprvllistid="'+response.resdta[i].id+'" >';dtfrmt=response.resdta[i].date_added.split("-");dtfrmtspace=response.resdta[i].date_added.split(" ");ddmmyy=dtfrmtspace[0];dtfrmt=dtfrmtspace[0].split("-");ddmmyy=dtfrmt[2]+'-'+dtfrmt[1]+'-'+dtfrmt[0];times=dtfrmtspace[1];addhtmlnxt+='<td width="15%" >'+ddmmyy+'  '+times+'</td>';addhtmlnxt+='<td width="20%">'+response.resdta[i].fullname+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].pan+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].cin+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].address+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].mobile+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].category+'</td>';addhtmlnxt+='<td width="15%">'+response.resdta[i].appntdate+'</td>';if(response.resdta[i].draft)
...
{addhtmlnxt+='<td width="15%"><button type="button" class="btn btn-primary" id="apprvrqst" formbid="'+response.resdta[i].id+'" pdfurl="'+draft+'">Approve</button></td>';}
else
...
{}});}
website('body').on('click','#apprvrqst',function(){var id=website(this).attr('formbid');var pdfurl=website(this).attr('pdfurl');var formdata={id:id,pdfurl:pdfurl};website.ajax({url:'sebi/apprvrqst',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{website('.preloder_wraper').fadeIn();},uploadProgress:function(event,position,total,percentComplete)
{website('.preloder_wraper').fadeIn();},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
...
{}});website('body').on('click','#formbrqust',function(){website('#Mymodalformb').modal('show');});website('body').on('click','#sendforaprv',function(){var formbid=website(this).attr('formbid');website('#myModalsendaprv .yesapprove').attr('formbid',formbid);website('#myModalsendaprv').modal('show');})
website('body').on('click','.yesapprove',function()
{var id=website(this).attr('formbid');var formdata={id:id};website.ajax({url:'sebi/sendforapprvlformb',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
...
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{website('#modaldocument .downloadpdf').hide();website('#modaldocument .docpdf').html(response.docontent);if(response.secutype)
{website.each(response.secutype,function(k,v)
{website('.secutype').append(v);});}
website('.name').html(response.formdata['fullname']);website('.cmpnme').html(response.formdata['companyname']);website('.pan').html(response.formdata['pan']);website('.cin').html(response.formdata['cin']);website('.contctno').html(response.formdata['mobile']);website('.address').html(response.formdata['address']);website('.category').html(response.formdata['category']);website('.appointdate').html(response.formdata['date']);website('.secuno').html(response.formdata['securityno']);website('.shrhldng').html(response.formdata['sharehldng']);website('.futureunit').html(response.formdata['futureunit']);website('.optionunit').html(response.formdata['optionunit']);website('.futurevalue').html(response.formdata['futurevalue']);website('.optionvalue').html(response.formdata['optionvalue']);website('#modaldocument #formbid').val(id);website('#modaldocument').modal('show');}
else
