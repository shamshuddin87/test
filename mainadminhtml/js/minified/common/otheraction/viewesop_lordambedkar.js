...
{var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var esopid=website('#esopid').val();var esopuniqueid=website('#esopuniqueid').val();var formdata={esopid:esopid,esopuniqueid:esopuniqueid,noofrows:noofrows,pagenum:pagenum}
website.ajax({url:'esop/fetchesopforview',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
...
website('.appendviewesop').html(addhtmlnxt);website("#empcount").html(response.empcount);website('.paginationmn').html(response.pgnhtml);}
else
...
{var uniqueid=website(this).attr('uniqueid');website('#myModalsavefinal #esopuniqid').val(uniqueid);website('#myModalsavefinal').modal('show');});website('body').on('click','.finalsaveyes',function(e)
{var esopuniq=website('#myModalsavefinal #esopuniqid').val();var formdata={esopuniq:esopuniq}
website.ajax({url:'esop/saveesopfinal',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
