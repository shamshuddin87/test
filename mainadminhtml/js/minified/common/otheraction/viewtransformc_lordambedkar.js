...
{},success:function(response,textStatus,jqXHR)
{console.log(response.pershare);if(response.logged===true)
...
website('.name').html(response.formdata['fullname']);website('.cmpnme').html(response.formdata['company_name']);website('.pan').html(response.formdata['pan']);website('.cin').html(response.formdata['cin']);website('.contctno').html(response.formdata['mobile']);website('.opngblnc').html(response.formdata['adrshldng']);website('.pershare').html(response.postnumber);website('.prepercent').html(response.prepercent);website('.postpercent').html(response.postpercent);website('.noofshares').html(response.formdata['tdsshare']);website('.totalamt').html(response.formdata['totalamnt']);website('.address').html(response.formdata['address']);website('.category').html(response.formdata['category']);website('.pretrans').html(response.formdata['pretrans']);website('.posttrans').html(response.formdata['posttrans']);website('.fromdate').html(ddmmyyfrom);website('.todate').html(ddmmyyto);website('.dateofintimtn').html(ddmmyyinti);website('.acquimode').html(response.formdata['acquistnmode']);website('.buyvalue').html(response.formdata['buyvalue']);website('.buynumbrunt').html(response.formdata['buynumbrunt']);website('.sellvalue').html(response.formdata['sellvalue']);website('.sellnumbrunt').html(response.formdata['sellnumbrunt']);website('.exetrd').html(response.formdata['exetrd']);website('#modaldocument #formcid').val(id);website('#modaldocument').modal('show');}
else
{}},complete:function(response)
{website('#myModalyesno .mainprogressbarforall').fadeOut();},error:function()
{}});});website('body').on('click','.formcpdf',function(e)
{var htmldata=website('#modaldocument .docpdf').html();var formcid=website('#modaldocument #formcid').val();var dataString=website("form[name=excelupload]").serializeArray();var formData={htmldata:htmldata,formcid:formcid};website.ajax({type:"POST",url:'sebi/generateformcPDF',data:formData,dataType:"json",beforeSend:function()
