...
{var rscrntpg=website('.gotobtn').val();website('.panel.panel-white #pagenum').val(rscrntpg);getdataonload();});website('body').on('click','#emp_status',function(e)
{getdataonload();});getdataonload();function getdataonload()
{var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var emp_status=website('#emp_status').val();var formdata={noofrows:noofrows,pagenum:pagenum,emp_status:emp_status};website.ajax({url:'mis/fetchallupsitypes',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
{var myhtml='';for(var i=0;i<response.resdta.length;i++)
{var j=i+1;var enddate=response.resdta[i]['enddate']?response.resdta[i]['enddate']:'';myhtml+='<tr class="getallups" upsid="'+response.resdta[i]['uppid']+'">';myhtml+='<td>'+j+'</td>';myhtml+='<td>'+response.resdta[i]['upsitype']+'</td>';myhtml+='<td>'+response.resdta[i]['projstartdate']+'</td>';myhtml+='<td>'+enddate+'</td>';myhtml+='<td>'+response.resdta[i]['dtadd']+'</td>';myhtml+='<td>'+response.resdta[i]['fullname']+'</td>';if(response.resdta[i].emp_status=='1')
{myhtml+='<td>Active</td>';}
...
{var baseHref=getbaseurl();var upsid=website(this).attr('upsid');window.location.href=baseHref+"mis/mis_infosharing?upsid="+upsid;});website('.genfile').on('click',function(e){var request=website(this).attr('request');var emp_status=website("#emp_status").val();var formdata={request:request,emp_status:emp_status}
