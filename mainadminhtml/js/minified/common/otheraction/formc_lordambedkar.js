...
addhtmlnxt+='<td width="20%">'+designation+'</td>';addhtmlnxt+='<td width="20%"><i class="fa fa-file-pdf-o" id="previewc" type="'+response.resdta[i].isitesop+'" doc_id=2 formcid="'+response.resdta[i].id+'" formctype="'+response.resdta[i].typeofformc+'"></i></td>';if(response.user_group_id=='7')
...
{var docid=website(this).attr('doc_id');var id=website(this).attr('formcid');var type=website(this).attr('type');var formctype=website(this).attr('formctype');var formdata={id:id,docid:docid,type:type,formctype:formctype};website.ajax({url:'sebi/previewofformc',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
...
website('.place').html(response.formdata['place']);website('.dateadded').html(finaldtadd);website('.contractspecific').html(response.formdata['contract_specification']);website('.excelcontractspecific').val(response.formdata['contract_specification']);website('.contracttype').html(response.formdata['contractname']);website('.excelcontracttype').val(response.formdata['contractname']);website('.posttrans').html(response.pershare);website('.excelposttrans').val(response.pershare);if(formctype=='1')
{if(response.formdata['type_of_transaction']==1)
{website('.transtype').html('<p style="color:green;">BUY</p>');website('.exceltranstype').html(response.formdata['typtrans']);}
else if(response.formdata['type_of_transaction']==2)
{website('.transtype').html('<p>SELL</p>');website('.exceltranstype').html(response.formdata['typtrans']);}
else
...
website('.name').html(response.formdata['fullname']);website('.excelname').val(response.formdata['fullname']);website('.cmpnme').html(response.formdata['company_name']);website('.excelcmpnme').val(response.formdata['company_name']);website('.pan').html(response.formdata['pan']);website('.excelpan').val(response.formdata['pan']);website('.cin').html(response.formdata['cin']);website('.excelcmpisin').val(response.formdata['cin']);website('.contctno').html(response.formdata['mobile']);website('.excelcontctno').val(response.formdata['mobile']);website('.opngblnc').html(response.formdata['sharehldng']);website('.excelopngblnc').val(response.formdata['sharehldng']);website('.pershare').html(response.postnumber);website('.excelpershare').val(response.postnumber);website('.prepercent').html(response.prepercent);website('.excelprepercent').val(response.prepercent);website('.postpercent').html(response.postpercent);website('.excelpostpercent').val(response.postpercent);website('.noofshares').html(response.formdata['tdsshare']);website('.excelnoofshares').val(response.formdata['tdsshare']);website('.totalamt').html(response.formdata['totalamnt']);website('.exceltotalamt').val(response.formdata['totalamnt']);website('.address').html(response.formdata['address']);website('.exceladdress').val(response.formdata['address']);website('.category').html(response.formdata['category']);website('.excelcategory').val(response.formdata['category']);website('.pretrans').html(response.formdata['pretrans']);website('.excelpretrans').val(response.formdata['pretrans']);website('.posttrans').html(response.formdata['posttrans']);website('.excelposttrans').val(response.formdata['posttrans']);website('.fromdate').html(ddmmyyfrom);website('.excelfromdate').val(ddmmyyfrom);website('.todate').html(ddmmyyto);website('.exceltodate').val(ddmmyyto);website('.dateofintimtn').html(finaldtadd);website('.exceldateofintimtn').val(ddmmyyinti);if(formctype=='1')
{website('.acquimode').html(response.formdata['acquistnmode']);website('.excelacquimode').val(response.formdata['acquistnmode']);}
else
{website('.acquimode').html(response.formdata['acquimode']);website('.excelacquimode').html(response.formdata['acquimode']);}
website('.buyvalue').html(response.formdata['buyvalue']);website('.excelbuyvalue').val(response.formdata['buyvalue']);website('.buynumbrunt').html(response.formdata['buynumbrunt']);website('.excelbuynumbrunt').val(response.formdata['buynumbrunt']);website('.sellvalue').html(response.formdata['sellvalue']);website('.excelsellvalue').val(response.formdata['sellvalue']);website('.sellnumbrunt').html(response.formdata['sellnumbrunt']);website('.excelsellnumbrunt').val(response.formdata['sellnumbrunt']);website('.exetrd').html(response.formdata['exetrd']);website('.excelexetrd').val(response.formdata['exetrd']);website('.formcidexcel').val(id);website('.exchange_on_trade_exec').html(response.formdata['fieldname']);website('.exchange_on_trade_exec').val(response.formdata['fieldname']);website('#modaldocument #formcid').val(id);website('#modaldocument').modal('show');}
else
