...
{var search=website('#srch').val();var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var startdate=website('#date1').val();var enddate=website('#date2').val();website.ajax({url:'mis/misconfirmtrde',data:{noofrows:noofrows,pagenum:pagenum,startdate:startdate,enddate:enddate,search:search},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{var htmlelements='';if(response.logged==true)
{for(var i=0;i<response.data.length;i++)
{var j=i+1;var tradewindw=response.data[i].trading_date.split("-");var tradewindwdte=new Date(tradewindw[2],tradewindw[1],tradewindw[0]);var trade=response.data[i].date_of_transaction.split("-");var tradedte=new Date(trade[2],trade[1],trade[0]);htmlelements+='<tr>';htmlelements+='<td width="10%">'+j+'</td>';htmlelements+='<td width="10%">'+response.data[i].fullname+'</td>';htmlelements+='<td width="10%">'+response.data[i].company_name+'</td>';htmlelements+='<td width="10%">'+response.data[i].preclrtrade+'</td>';if(response.data[i].approved_date!=null)
