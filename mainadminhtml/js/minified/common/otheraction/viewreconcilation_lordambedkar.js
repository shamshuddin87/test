...
{var addhtmlnxt='';var email=new Array();var name=new Array();var final=new Array();for(var i=0;i<response.resdta['data'].length;i++)
...
{email.push(response.resdta['data'][i].email);name.push(username);final.push(diffrnc);website("#sendmail").modal('show');website("#emailid").val(email);website("#name").val(name);website("#diffrnc").val(final);addhtmlnxt+='<tr class="counter" reconciid="'+response.resdta['data'][i].id+'" style="background-color:#f5aaaa;">';}
else
