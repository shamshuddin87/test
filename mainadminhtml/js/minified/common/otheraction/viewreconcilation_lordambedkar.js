...
{var pan=response.resdta['data'][i].panno?response.resdta['data'][i].panno:'';var username=response.resdta['username'][i]?response.resdta['username'][i]:'';var relativename=response.resdta['data'][i].relativename?response.resdta['data'][i].relativename:'';var relationship=response.resdta['data'][i].relationship?response.resdta['data'][i].relationship:'';var script=response.resdta['data'][i].script?response.resdta['data'][i].script:'';var holding=response.resdta['data'][i].holding?response.resdta['data'][i].holding:'';var equity=response.resdta['data'][i].shares?response.resdta['data'][i].shares:'';var diffrnc=Number(holding)-Number(equity);if(diffrnc!=0)
{email=response.resdta['data'][i].email;addhtmlnxt+='<tr class="counter" reconciid="'+response.resdta['data'][i].id+'" style="background-color:#f5aaaa;">';}
else
{addhtmlnxt+='<tr class="counter" reconciid="'+response.resdta['data'][i].id+'">';}
addhtmlnxt+='<td width="5%"><input type="checkbox" name="emailcheckbox" class="emailcheckbox" value="'+email+'"  username ="'+username+'"  diffrnc ="'+diffrnc+'"><br></td>';addhtmlnxt+='<td width="15%">'+pan+'</td>';addhtmlnxt+='<td width="10%">'+username+'</td>';addhtmlnxt+='<td width="15%">'+relativename+'</td>';addhtmlnxt+='<td width="10%">'+relationship+'</td>';addhtmlnxt+='<td width="15%">'+script+'</td>';addhtmlnxt+='<td width="15%">'+holding+'</td>';addhtmlnxt+='<td width="15%">'+equity+'</td>';addhtmlnxt+='<td width="30%">'+diffrnc+'</td>';addhtmlnxt+='</tr>';}
for(var i=0;i<response.panuser.length;i++)
{var pansheet=response.panlist[i]?response.panlist[i]:'';var pan=response.panuser[i].panno?response.panuser[i].panno:'';var username=response.panuser[i].username?response.panuser[i].username:'';var relativename=response.panuser[i].relativename?response.panuser[i].relativename:'';var relationship=response.panuser[i].relationship?response.panuser[i].relationship:'';if(jQuery.inArray(response.panuser[i].panno,response.panlist)=='-1')
{addhtmlnxt+='<tr class="counter" reconciid="'+response.panuser[i].id+'">';addhtmlnxt+='<td width="5%"><input type="checkbox" name="emailcheckbox" class="emailcheckbox" value="'+email+'" username ="'+username+'"  diffrnc ="'+diffrnc+'" ><br></td>';addhtmlnxt+='<td width="15%">'+pan+'</td>';addhtmlnxt+='<td width="10%">'+username+'</td>';addhtmlnxt+='<td width="15%">'+relativename+'</td>';addhtmlnxt+='<td width="10%">'+relationship+'</td>';addhtmlnxt+='<td width="15%"></td>';addhtmlnxt+='<td width="15%"></td>';addhtmlnxt+='<td width="15%"></td>';addhtmlnxt+='<td width="30%"><input type="submit" value="Error! Message" class="showerror" title="error message"></td>';addhtmlnxt+='</tr>';}}
...
{website('#myModalerrormssage').modal('show');});website("body").on("click","#emailExcelToBenpose",function(e){var email=[];var name=[];var difference=[];website(".emailcheckbox:checked").each(function(){email.push(website(this).val());});website(".emailcheckbox:checked").each(function(){name.push(website(this).attr('username'));});website(".emailcheckbox:checked").each(function(){difference.push(website(this).attr('diffrnc'));});website("#emailid").val(email);website("#name").val(name);website("#diffrnc").val(difference);website("#sendmail").modal("show");});website("body").on("click",".getallchkbox",function(e){if(website(this).is(":checked"))
