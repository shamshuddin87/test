...
{getmisnonexetrde();});getmisnonexetrde();function getmisnonexetrde()
{var search=website('#srch').val();var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var emp_status=website('#emp_status').val();website.ajax({url:'mis/misnonexetrde',data:{noofrows:noofrows,pagenum:pagenum,search:search,emp_status:emp_status},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{var htmlelements='';if(response.logged==true)
{for(var i=0;i<response.data.length;i++)
{var j=i+1;var apprvdte=response.data[i].approved_date?response.data[i].approved_date:'';htmlelements+='<tr>';htmlelements+='<td width="10%">'+j+'</td>';htmlelements+='<td width="10%">'+response.data[i].fullname+'</td>';if(response.data[i].emp_status=='1')
...
htmlelements+='<td width="10%">'+response.data[i].no_of_shares+'</td>';htmlelements+='<td width="10%">'+response.data[i].date_added+'</td>';htmlelements+='</tr>';}}
else
