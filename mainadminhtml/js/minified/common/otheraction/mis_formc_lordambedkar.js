...
{var urlredirect=website('#filterstatus').val();website('#redirecturl').val(urlredirect);getmisformc("");});datepicker();function datepicker(){website('.bootdatepick').datetimepicker({weekStart:1,todayBtn:0,autoclose:1,todayHighlight:0,startView:2,minView:2,forceParse:0,format:"yyyy-mm-dd"}).on('change',function(e,date)
...
var search=website('#srch').val();var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var startdate=website('#date1').val();var enddate=website('#date2').val();var filterstatus=website('#filterstatus').val();website.ajax({url:'mis/misformc',data:{noofrows:noofrows,pagenum:pagenum,search:search,startdate:startdate,enddate:enddate,filterstatus:filterstatus},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{var htmlelements='';if(response.logged==true)
{for(var i=0;i<response.data.length;i++)
{var formcsenddate=response.data[i].send_date?response.data[i].send_date:'';var date=response.data[i].date_of_transaction;var submsndate=new Date(date.split("-").reverse().join("-"));submsndate.setDate(submsndate.getDate()+2);var dd=("0"+submsndate.getDate()).slice(-2);var mm=("0"+(submsndate.getMonth()+1)).slice(-2);var y=submsndate.getFullYear();var duesubDate=dd+'-'+mm+'-'+y;var j=i+1;htmlelements+='<tr>';htmlelements+='<td width="10%">'+j+'</td>';htmlelements+='<td width="10%">'+response.data[i].fullname+'</td>';htmlelements+='<td width="10%">'+response.data[i].company_name+'</td>';htmlelements+='<td width="10%">'+response.data[i].date_of_transaction+'</td>';htmlelements+='<td width="10%">'+response.data[i].no_of_share+'</td>';htmlelements+='<td width="10%">'+duesubDate+'</td>';htmlelements+='<td width="10%">'+formcsenddate+'</td>';if(response.data[i].send_date)
