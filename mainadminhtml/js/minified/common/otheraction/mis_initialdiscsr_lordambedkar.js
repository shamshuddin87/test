...
{getinitialdisclsr();});website('body').on('change','#emp_status',function(e)
...
{var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var filterstatus=website('#filterstatus').val();var search=website('#srch').val();var emp_status=website('#emp_status').val();website.ajax({url:'mis/pendinginitialdisclsr',data:{noofrows:noofrows,pagenum:pagenum,search:search,filterstatus:filterstatus,emp_status:emp_status},method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{var htmlelements='';if(response.logged==true)
{for(var i=0;i<response.data.length;i++)
{var j=i+1;htmlelements+='<tr>';var sent_date=response.data[i].sent_date?response.data[i].sent_date:'';htmlelements+='<td width="10%">'+j+'</td>';htmlelements+='<td width="10%">'+response.data[i].fullname+'</td>';if(response.data[i].emp_status=='1')
...
htmlelements+='<td width="10%">'+response.data[i].dpdate+'</td>';htmlelements+='<td width="10%">'+sent_date+'</td>';if(response.data[i].sent_date)
