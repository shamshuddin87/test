...
{var trdeplnids=website(this).attr('tradeplanid');trdeplnids=btoa(trdeplnids);var uniquid=website(this).attr('uniquid');uniquid=btoa(uniquid);var baseHref=getbaseurl();window.location.href=baseHref+'tradingplan/tradeplanview?tradeid='+trdeplnids+'&&uniqueid='+uniquid+'';});website('body').on('click','.approveplan',function()
{var tradeid=website(this).attr('idss');var formdata={tradeid:tradeid}
website.ajax({url:'tradingplan/apprvtradeplan',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
...
{var tradeid=website(this).attr('idss');website('#tradeplanmodal #rejecttradeplan').attr('rejecttrdeid',tradeid);website('#tradeplanmodal').modal('show');});website('body').on('click','#rejecttradeplan',function()
{var rejctid=website(this).attr('rejecttrdeid');var messg=website('#rejectplantrade').val();var formdata={rejctid:rejctid,messg:messg}
website.ajax({url:'tradingplan/rejcttradeplan',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
