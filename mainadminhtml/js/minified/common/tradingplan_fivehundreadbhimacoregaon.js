...
{website('#reltv').hide();website('#specfdte').hide();website('#dtrngfrm').hide();website('#dterngto').hide();var date=new Date();var newdate=new Date(date);newdate.setDate(newdate.getDate());var dd=("0"+newdate.getDate()).slice(-2);var mm=("0"+(newdate.getMonth()+1)).slice(-2);var y=newdate.getFullYear();var periodfromdate=dd+'-'+mm+'-'+y;website('#frmdate').val(periodfromdate);var dateto=new Date(date);dateto.setDate(dateto.getDate()+365);var dd=("0"+dateto.getDate()).slice(-2);var mm=("0"+(dateto.getMonth()+1)).slice(-2);var y=dateto.getFullYear();var periodtodate=dd+'-'+mm+'-'+y;website('#todate').val(periodtodate);website('.bootdatepick').datetimepicker({weekStart:1,todayBtn:0,autoclose:1,todayHighlight:0,startView:2,minView:2,forceParse:0,format:"dd-mm-yyyy"}).on('change',function(e,date)
...
{var rscrntpg=website('.gotobtn').val();website('.panel.panel-white #pagenum').val(rscrntpg);getdataonload();});function loaddatepick()
{var date=new Date();var newdate=new Date(date);newdate.setDate(newdate.getDate());var dd=("0"+newdate.getDate()).slice(-2);var mm=("0"+(newdate.getMonth()+1)).slice(-2);var y=newdate.getFullYear();var periodfromdate=y+'-'+mm+'-'+dd;website('.appendtradingplan #frmdate').val(periodfromdate);var dateto=new Date(date);dateto.setDate(dateto.getDate()+365);var dd=("0"+dateto.getDate()).slice(-2);var mm=("0"+(dateto.getMonth()+1)).slice(-2);var y=dateto.getFullYear();var periodtodate=y+'-'+mm+'-'+dd;website('.appendtradingplan #todate').val(periodtodate);website('.bootdatepick').datetimepicker({weekStart:1,todayBtn:0,autoclose:1,todayHighlight:0,startView:2,minView:2,forceParse:0,format:"dd-mm-yyyy"}).on('change',function(e,date)
...
{return false;}});website('#inserttradingplan').ajaxForm({dataType:"json",beforeSend:function()
...
{var noofrows=website('#noofrows').val();var pagenum=website('#pagenum').val();var formdata={noofrows:noofrows,pagenum:pagenum};website.ajax({url:'tradingplan/fetchtradeplan',data:formdata,method:'POST',contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
{if(response.logged===true)
...
addhtmlnxt+='<td width="15%" class="trade"><i class="fa fa-eye faicon floatleft edittradeplan" title="View entry" id="edittrade" tradeplanid="'+response.resdta[i].id+'" uniquid="'+response.resdta[i].uniqueid+'"></i></td>';addhtmlnxt+='</tr>';}
website('.appendtradeplan').html(addhtmlnxt);website('.paginationmn').html(response.pgnhtml);}
else
{website('.paginationmn').html(response.pgnhtml);}},complete:function(response)
{website('.preloder_wraper').fadeOut();},error:function(jqXHR,textStatus,errorThrown)
{}});}
website('body').on('click','.edittradeplan',function()
{var trdeplnids=website(this).attr('tradeplanid');trdeplnids=btoa(trdeplnids);var uniquid=website(this).attr('uniquid');uniquid=btoa(uniquid);var baseHref=getbaseurl();window.location.href=baseHref+'tradingplan/tradeplanview?tradeid='+trdeplnids+'&&uniqueid='+uniquid+'';});website('body').on('click','.showrejctmsg',function(e)
