...
{},error:function(jqXHR,textStatus,errorThrown)
{}});});website("body").on("click",".relaccedit",function(e){var relaccno=website(this).attr('relaccno');var dp=website(this).attr('dp');var ch=website(this).attr('ch');var name=website(this).attr('relname');var editid=website(this).attr('relacedit')
var nationality=website(this).attr(nationality);alert(nationality);website('#releditaccmodal #dparrel').val(dp);website('#releditaccmodal #relclhouse').val(ch);website('#releditaccmodal #reledname').val(name);website('#releditaccmodal #releditaccno').val(relaccno);website('#releditaccmodal .relupacc').attr('btnedit',editid);website('#releditaccmodal').modal('show');});website("body").on("click",".relupacc",function(e){var reledit=website(this).attr('btnedit');var accno=website("#releditaccno").val();var dp=website("#dparrel").val();var ch=website("#relclhouse").val();website.ajax({url:'portfolio/updaterelacc',method:'POST',data:{reledit:reledit,accno:accno,dp:dp,ch:ch},contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
