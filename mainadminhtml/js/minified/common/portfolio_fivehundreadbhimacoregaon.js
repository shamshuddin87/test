...
else{new PNotify({title:'Alert',text:"No Of Accounts Must Be Less Than 10",type:'university',hide:true,styling:'bootstrap3',addclass:'dark ',});}});website("body").on("click","#subdemat",function(e){var len=website('.acsub').length;var self_nation=website('#self_nation').val();var mydata=[];for(var i=1;i<=len;i++)
{var accno=website('#field_'+i).val();var dp=website('#field2_'+i).val();var obj={"accno":accno,"dp":dp,};mydata.push(obj);}
...
website('#editaccmodal .upacc').attr('btnedit',editid);website('#editaccmodal').modal('show');});website('body').on("click",".upacc",function(e){var accno=website('#editaccno').val();var rp=website('#dpar').val();var hc=website('#clhouse').val();var self_nation=website("#self_nation").val();var editid=website(this).attr('btnedit');website.ajax({url:'portfolio/updateacc',method:'POST',data:{accno:accno,editid:editid,rp:rp,hc:hc,self_nation:self_nation},contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
...
{},error:function(jqXHR,textStatus,errorThrown)
{}});})
website("body").on("click",".relhtml",function(e){var noofacc=website('#reldematno').val();var relinfo=website('#relinfo').val();var nationality=website("#relinfo option:selected").attr("nationality");if(relinfo!='')
{var no=website('#reldematno').val();if(no<=10){var myhtml=' <table class="table table-inverse" id="datableabhi"><tr><th>Account No </th><th>Depository Participant </th></tr>';for(var i=1;i<=no;i++)
...
{var txtdata=website('#relfield_'+i).val();var dp=website('#relfield2_'+i).val();if(txtdata!=''&&dp!=''&&ch!='')
{var obj={relativeacc:txtdata,dp:dp};myarr.push(obj);}
else
...
{htmlelements+='<tr>';htmlelements+='<td>'+j+'</td>';htmlelements+='<td>'+response.data[i].name+'</td>';htmlelements+='<td>'+response.data[i].accountno+'</td>';htmlelements+='<td>'+response.data[i].depository_participient+'</td>';htmlelements+='<td><i class="fa fa-edit relaccedit" relname="'+response.data[i].name+'" dp="'+response.data[i].depository_participient+'" ch="'+response.data[i].clearing_house+'" relaccno="'+response.data[i].accountno+'" relacedit="'+response.data[i].id+'"  nationality="'+response.data[i].nationality+'" ></i>'+'<i class="fa fa-trash relaccdel" acourel="'+response.data[i].id+'" ></i></td>';htmlelements+='</tr>';j=j+1;}}
...
{},error:function(jqXHR,textStatus,errorThrown)
{}});});website("body").on("click",".relaccedit",function(e){var relaccno=website(this).attr('relaccno');var dp=website(this).attr('dp');var ch=website(this).attr('ch');var name=website(this).attr('relname');var editid=website(this).attr('relacedit');var nationality=website(this).attr('nationality');website('#releditaccmodal #dparrel').val(dp);website('#releditaccmodal #relclhouse').val(ch);website('#releditaccmodal #relednation').val(nationality);website('#releditaccmodal #reledname').val(name);website('#releditaccmodal #releditaccno').val(relaccno);website('#releditaccmodal .relupacc').attr('btnedit',editid);if(nationality=='Other')
{website('#releditaccmodal #releditaccno').removeAttr("maxlength");}
else
{website('#releditaccmodal #releditaccno').attr("maxlength","16");}
website('#releditaccmodal').modal('show');});website("body").on("click",".relupacc",function(e){var reledit=website(this).attr('btnedit');var accno=website("#releditaccno").val();var dp=website("#dparrel").val();var ch=website("#relclhouse").val();let rel_nation=website("#relednation").val();website.ajax({url:'portfolio/updaterelacc',method:'POST',data:{reledit:reledit,accno:accno,dp:dp,ch:ch,rel_nation:rel_nation},contentType:'application/x-www-form-urlencoded; charset=UTF-8',dataType:"json",cache:false,beforeSend:function()
{},uploadProgress:function(event,position,total,percentComplete)
{},success:function(response,textStatus,jqXHR)
...
{},error:function(jqXHR,textStatus,errorThrown)
{}});})
relativeaccinfo();website("body").on("click","#subreldemat",function(e){var nooffield=website('.relac').length;var relinfo=website('#relinfo').val();var nationality=website("#relinfo option:selected").attr("nationality");var myarr=[];for(var i=1;i<=nooffield;i++){var txtdata=website('#relfield_'+i).val();var dp=website('#relfield2_'+i).val();var ch=website('#relfield3_'+i).val();if(txtdata!=''&&dp!=''&&ch!=''){var obj={relativeacc:txtdata,dp:dp,ch:ch};myarr.push(obj);}
