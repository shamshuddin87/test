      ...
      success: function(response, textStatus, jqXHR)
      {
        if(response.logged===true)
        {
            ...
            for(var i = 0; i < response.resdta.length; i++) 
            {
                     ...
                     yymmdd = dtfrmt[0]+'-'+dtfrmt[1]+'-'+dtfrmt[2];
                     times = dtfrmtspace[1];   
...
{
    var esopid = website(this).attr('esopid');
    var uniquid = website(this).attr('uniqueid');
    uniquid = btoa(uniquid);
    esopid = btoa(esopid);
    var baseHref = getbaseurl(); 
    window.location.href=baseHref+'esop/viewesop?esopid='+esopid+'&&uniqueid='+uniquid+''; 
});
