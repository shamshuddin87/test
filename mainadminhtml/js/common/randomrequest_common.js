...
{ 
    var rqid = website('#rqst').val();
    var vote = website('#vote').val();
    var requserid=website('#requserid').val();
    // alert(requserid);return false;
    if(vote == 1)
    {
        var formdata = {rqid:rqid,vote:vote,requserid:requserid}
        website.ajax({
              url:'randomrequest/updaterequst',
              data:formdata,
              method:'POST',
              //contentType:'json',
              ...
              success: function(response, textStatus, jqXHR)
              {
                  if(response.logged===true)
                  {
                         ...
                         sendmailackreqstr(response.rqstid);
                  }
              },
    ...
    else
    {
        ...
        website('#updaterqststatus').ajaxForm({
        //data:formdata,
        //contentType:'application/x-www-form-urlencoded; charset=UTF-8',
        dataType:"json",
        beforeSend: function() 
...
function sendmailackreqstr(rqstid)
{
    var formdata = {rqstid:rqstid}
     website.ajax({
              url:'randomrequest/mailacknwtoapprvr',
              data:formdata,
              method:'POST',
              //contentType:'json',
              contentType:'application/x-www-form-urlencoded; charset=UTF-8',
