...
function confirmdisclosure(id) {
  ...
  if (id == "confirmpersonalinfo") {
    ...
    var nationality = website("#per_nation").val();
...
website("body").on("click", "#yesdisclosures1", function (e) {
  ...
  var formdata = {
    ...
    rqid: rqid,
    toemail: toemail,
    shareholdng: shareholdng,
    adrsholdng: adrsholdng,
      ...
      if (response.logged == true) {
        ...
        if (filepath) {
          myhtml +=
            '<td><a href="' +
            filepath +
            '" download>&nbsp;<i class="fa fa-download" id="uploadattached1" aria-hidden="true"></i></a></td>';
        } else {
          myhtml += "<td></td>";
        }

        myhtml +=
          '<td><i class="fa fa-eye faicon floatleft viewdetail" title="View entry" id="editdetail" personalid="' +
          response.data.id +
          '"></i></td>';
        if (response.data.send_status == 1) {
          ...
          myhtml +=
            '<td><i class="fa fa-thumbs-o-up sendfrapp" style="font-size:20px"></i></td>';
        }
        ...
        if (response.data.approved_status == 1) {
          ...
          response.data.approved_status == 0 &&
          response.data.approved_status != ""
        ) {
          myhtml += '<td><p style="color:red;">Rejected</p></td>';
        } else if (
          response.data.send_status == 1 &&
          response.data.approved_status == ""
        ) {
        ...
        if (
          response.data.send_status == 1 &&
          response.data.approved_status == 1
        ) {
          myhtml += "<td>---</td>";
        } else {
          myhtml +=
            '<td><i class="fa fa-edit editcmp" editid="' +
            id +
            '"  style=""></i><i class="fa fa-trash del" delid="' +
            id +
            '"  style=""></i></td>';
        }
      } else {
...
website("body").on("click", ".addrelinfo", function (e) {
  //on click addrelinfo one more form added
  ...
  //-------------------------------------------------Create Form--------------------------------------//
  myhtml =
    '<form method="post" id="getdata_' +
    newno +
    '" class="chklength"  enctype="multipart/form-data"><div class="input-group col-md-12 col-xs-12 col-sm-12 remdiv_' +
    newno +
    '">' +
    '<div class="col-md-3">' +
    "<label>Full name</label>" +
    '<input  class="form-control " placeholder="full name" id="' +
    newno +
    '_fname"  name="' +
    newno +
    '_fname" type="text" />' +
    ...
    '<div class="col-md-3">' +
    "<label>Pan</label>" +
    '<input  class="form-control panval"  placeholder="pan" id="' +
    newno +
    '_pan" name="' +
    newno +
    '_pan" type="text" />' +
    ...
    '<div class="col-md-3">' +
    "<label>Aadhar</label>" +
    '<input  class="form-control aadhar"  placeholder="aadhar" id="' +
    newno +
    '_aadhar" name="' +
    newno +
    '_aadhar" type="text" />' +
    ...
    '<div class="col-md-3">' +
    "<label>Age</label>" +
    '<input  class="form-control " placeholder="age" id="' +
    newno +
    '_age" name="' +
    newno +
    '_age" type="text" />' +
    ...
    '_sex" name="' +
    newno +
    '_sex" value="Male" checked/>Male' +
    '<input type="radio" id="' +
    newno +
    '_sex" name="' +
    newno +
    '_sex"  value="Female"/>Female ' +
    ...
    '_dob" id="' +
    newno +
    '_dob" class="bootdatepick" placeholder="dob">' +
    "</div>" +
    '<div class="col-md-3">' +
    "<lable>Relationship</lable>" +
    '<input  class="form-control " placeholder="relationship" id="' +
    newno +
    '_relationship"  name="' +
    newno +
    '_relationship" type="text" />' +
    "</div>" +
    '<div class="col-md-3">' +
    "<label >Address</label>" +
    '<textarea class="form-control " placeholder="address" id="' +
    newno +
    '_addr"  name="' +
    newno +
    '_address" type="text"></textarea>' +
    "</div>" +
    "</form>";
  var buttonhtml =
    '<div class="valida"><span class="glyphicon glyphicon-plus addrelinfo" style="color:green;" formno="' +
    newno +
    '"></span><span class="glyphicon glyphicon-minus deleterelinfo" style="color:red;" formno="' +
    newno +
    '"></span></div>';
  //-------------------------remove button------------------------------------------------//
...
website("body").on("click", ".deleterelinfo", function (e) {
  //on click minus remove last form
  var myform = website(this).attr("formno"); //get last form no
  var newno = myform - 1; //set new button no
  if (newno >= 1) {
    //if form is greater then 1 then remove it
    website(".addrelinfo").remove();
    website(".deleterelinfo").remove();

    var buttonhtml =
      '<span class="glyphicon glyphicon-plus addrelinfo" style="color:green;" formno="' +
      newno +
      '"></span><span class="glyphicon glyphicon-minus deleterelinfo" style="color:red;" formno="' +
      newno +
      '"></span>';
      ...
      if (response.logged === true) {
        ...
        for (var i = 0; i < response.data.length; i++) {
          var education = response.data[i].education
            ...
            : "";
          var sharehldng = response.data[i].sharehldng
            ? response.data[i].sharehldng
            : "";
          var adrshldng = response.data[i].adrshldng
            ? response.data[i].adrshldng
            : "";
          myhtml += "<tr>";
          myhtml += "<td>" + j + "</td>";
          ...
          myhtml +=
            '<td><i class="fa fa-edit editrel" releditid="' +
            response.data[i].id +
            '" style=""></i><i class="fa fa-trash delrel"  reldelid="' +
            response.data[i].id +
            '" style=""></i></td>';
          myhtml += "</tr>";
          j++;
...
website("body").on("click", ".delrel", function (e) {
  //on click submit button get data
  ...
  website.ajax({
    ...
    success: function (response, textStatus, jqXHR) {
      if (response.logged == true) {
        ...
        var relationship = response.data.relationship
        ...
        var sharehldng = response.data.sharehldng
          ? response.data.sharehldng
          : "";
        var adrshldng = response.data.adrshldng ? response.data.adrshldng : "";
        var legal_idntfr = response.data.legal_identifier
          ? response.data.legal_identifier
          : "";
        var legal_idntfctn_no = response.data.legal_identification_no
          ? response.data.legal_identification_no
          : "";
        var dependantnature = response.data.dependency_nature
          ? response.data.dependency_nature
          : "";
        var occupation = response.data.occupation
...
website("body").on("click", ".delmfr", function (e) {
  //on click submit button get data
...
website("body").on("click", ".mymfredit", function (e) {
  //on click submit button get data
...
function doSearch(getvalue) {
  ...
  if (website.trim(getkeyword) == "") {
    website("#live-search-header-wrapper ul").html(
      '<li class="noresultfound">No Result Fould!!!!</li>'
    );
  } else {
    ...
    website.ajax({
      ...
      uploadProgress: function (event, position, total, percentComplete) {
        ...
        website(".mainprogressbarforall .progress").fadeIn();
        website(".mainprogressbarforall .progress .progress-bar").width(
          percentComplete + "%"
        );
      },
      success: function (response, textStatus, jqXHR) {
        ...
        if (response.logged == true && response.data.length >= 1) {
          ...
          website("#live-search-header-wrapper ul").html(
            '<li class="noresultfound"><span class="resp_new">' +
              response.message +
              "</span></li>"
          );
        }
  ...
  if (website.trim(getkeyword) == "") {
    ...
    website.ajax({
      ...
      uploadProgress: function (event, position, total, percentComplete) {
        ...
        website("#Mymodaledit .mainprogressbarforall .progress").fadeIn();
        website(
          "#Mymodaledit .mainprogressbarforall .progress .progress-bar"
  ...
  website.ajax({
    ...
    success: function (response, textStatus, jqXHR) {
      if (response.logged === true) {
        ...
        var myhtml = "";
        for (var i = 0; i < response.data.length; i++) {
          var transctndate = response.data[i].transaction_date
            ? response.data[i].transaction_date
            : "";
          myhtml += "<tr>";
          myhtml += "<td>" + j + "</td>";
          myhtml += "<td>" + response.data[i].related_party + "</td>";
          ...
          myhtml +=
            '<td><i class="fa fa-edit trdintmtnedit"  mfredit="' +
            response.data[i].id +
            '" style="font-size:20px;"></i><i class="fa fa-trash deltrdintmtn"  trdintmtndel="' +
            response.data[i].id +
            '" style="font-size:20px;"></i></td>';
          myhtml += "</tr>";
          j++;
    ...
    success: function (response, textStatus, jqXHR) {
      if (response.logged == true) {
        ...
        if (response.data[0].transctn_type == 5) {
          ...
          website("#Mymodaledit #transdate").val(
            response.data[0].transaction_date
          );
        }
...
website("body").on("click", ".add_button", function (e) {
  ...
  if (getno <= 5) {
    ...
    htmlele +=
      '<section class="col col-md-12 col-xs-12 company_asses" id="pstbtn">';
    htmlele +=
      '<input type="button" value="Submit" class="btn btn-primary contractexcelbtn" id="pastbtnsub">' +
      "</section></div>";
    ...
    success: function (response, textStatus, jqXHR) {
      if (response.logged === true) {
        ...
        for (var i = 0; i < response.resdta.length; i++) {
          response.resdta;
          var emp_name = response.resdta[i].emp_name
            ? response.resdta[i].emp_name
            : "";
          var emp_desigtn = response.resdta[i].emp_desigtn
            ? response.resdta[i].emp_desigtn
            : "";
          var startdate = response.resdta[i].startdate
            ? response.resdta[i].startdate
            : "";
          var enddate = response.resdta[i].enddate
            ? response.resdta[i].enddate
            : "";
          htmlele +=
            '<div class="box_css">' +
            ...
            i +
            '" name="empid" class="form_fields form-control col-md-7 col-xs-12 empnm"  value= "' +
            response.resdta[i].id +
            '"  empid ="' +
            response.resdta[i].id +
            '"  style = "display:none;">' +
            '<input type="text" id="empname_' +
            i +
            '" name="empname" class="form_fields form-control col-md-7 col-xs-12 empnm" value= "' +
            emp_name +
            '" empname= "' +
            emp_name +
            '" required>' +
            ...
            '<input type="text" id="designtn_' +
            i +
            '" name="designtn" class="form_fields form-control col-md-7 col-xs-12 desig" value= "' +
            emp_desigtn +
            '" emp_desigtn= "' +
            emp_desigtn +
            '" required>' +
        ...
        htmlele +=
          '<section class="col col-md-12 col-xs-12 company_asses" id="pstbtn">';
        htmlele +=
          '<input type="button" value="Update" class="btn btn-primary contractexcelbtn" id="pastupdate">' +
          "</section>";
...
website("body").on("click", "#pastupdate", function (e) {
  var nooffield = website(".empnm").length;
  ...
  for (var i = 0; i < nooffield; i++) {
    ...
    if (empname != "" && designtn != "" && strtdte != "" && enddte != "") {
      ...
      website("#alertcommon #allalertmsg").html(
  ...
  if (myarr.length >= 1) {
    website.ajax({
      url: "employeemodule/updatepastemp",
      method: "POST",
      data: { myarr: myarr },
      contentType: "application/x-www-form-urlencoded; charset=UTF-8",
      //default: 'application/x-www-form-urlencoded; charset=UTF-8' ,'multipart/form-data' , 'text/plain'
