...
website('.bootdatepick').datetimepicker({
  ...
  function changeFunc() {
    var selectBox = document.getElementById("dresign");
    ...
    website.ajax({
        ...
        success: function(response, textStatus, jqXHR) 
        {
            ...
            if(response.logged==true)
            {
                ...
                {  
                    var j=i+1;
                    var resign=response.data[i].dpdate?response.data[i].dpdate:'';
                    htmlelements+='<tr>';
                    ...
                    htmlelements+='<td width="10%">'+response.data[i].date_modified+'</td>';
                    htmlelements+='</tr>';
                }
            }
            else
