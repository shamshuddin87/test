website(document).ready(function()
{
   ...
   website('#insertformd').ajaxForm({
      ...
      {  website('#Mymodalformb').modal('hide'); website('.preloder_wraper').fadeOut(); },
...
//            //------------------------- Table Fields Insertion START ------------------------
           ...
           addhtmlnxt += '<td width="15%"><i class="fa fa-file-pdf-o" id="previewd" doc_id=3 formdid="'+response.resdta[i].id+'"></i></td>';
            if(response.user_group_id == '7')
            {
...
website('body').on('click','.editformd', function(){
var id = website(this).attr('formdid');
    var formdata = {id:id};
    website.ajax({
      url:'sebi/fetchformdedit',
      data:formdata,
      method:'POST',
      //contentType:'json',
      contentType:'application/x-www-form-urlencoded; charset=UTF-8',
      ...
      success: function(response, textStatus, jqXHR)
      {
        if(response.logged===true)
        {
            ...
            website('#updateformd #upformdid').val(id);
...
website('body').on('click','#formdrqust', function(){
    var categry = website('').val();
    website('#Mymodalformc').modal('show');
    website('#Mymodalformc').modal('show');
});

website('body').on('click','#sendforaprvformd',function(){
    var formdid = website(this).attr('formdid');
    var pdfurl = website(this).attr('pdfurl');
    website('#myModalsendaprv .yesapprove').attr('formdid',formdid);
...
website('body').on('click','#previewd',function()
{
   var docid = website(this).attr('doc_id');
    ...
    website.ajax({
      ...
      //async:true, /*Cross domain checking*/
      beforeSend: function()
      {  website('#modaldocument .formdpdf').show(); },
      uploadProgress: function(event, position, total, percentComplete)
      {   },
        success: function(response, textStatus, jqXHR) 
        {
           if(response.logged === true)
           {
               ...
               website('.category').html(response.formdata['cmpcnct']);
               ...
               website('.fromdate').html(response.formdata['fromdate']);
               website('.todate').html(response.formdata['todate']);
               website('.dateofintimtn').html(response.formdata['dateofintimtn']);
...
website('body').on('click','.formdpdf', function(e)
{
    ...
    var formdid = website('#modaldocument #formdid').val();
    var formData = {htmldata:htmldata,formdid:formdid};
    website.ajax({
        type:"POST",
        url:'sebi/generateformdPDF',
        data: formData,
        ...
        success: function(response) 
        {
            ...
            {
                website('.preloder_wraper').fadeOut();
                website('#modaldocument .formdpdf').fadeOut();
