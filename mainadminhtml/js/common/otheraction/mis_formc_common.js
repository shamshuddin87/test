...
{
    var urlredirect = website('#filterstatus').val();
    website('#redirecturl').val(urlredirect);
    getmisformc("");
});
...
getmisformc(url_status);

function getmisformc(url_status)
{
    var urlredirect = website('#redirecturl').val();
    if(urlredirect) 
    {
      var status = urlredirect;
    ...
    website.ajax({
        url:'mis/misformc',
        data:{noofrows:noofrows,pagenum:pagenum,search:search,startdate:startdate,enddate:enddate,filterstatus:filterstatus,emp_status:emp_status},
        method:'POST',
        ...
        success: function(response, textStatus, jqXHR) 
        {
            ...
            if(response.logged==true)
            {
                ...
                {
                    var formcsenddate = response.data[i].send_date?response.data[i].send_date:'';
                    var date= response.data[i].date_of_transaction;
                    var submsndate=new Date(date.split("-").reverse().join("-"));
                    submsndate.setDate(submsndate.getDate() +2);
                    var dd = ("0" + submsndate.getDate()).slice(-2);
                    var mm = ("0" + (submsndate.getMonth() + 1)).slice(-2);
                    var y = submsndate.getFullYear();
                    var duesubDate = dd + '-' + mm + '-' + y;
                    var j=i+1;
                    htmlelements+='<tr>';
                    ...
                    {
                        var senddte = response.data[i].send_date.split("-");
                        var formsenddte = new Date(senddte[2], senddte[1], senddte[0]);
                    }
                    var duetrade = duesubDate.split("-");
                    var duetradedte = new Date(duetrade[2], duetrade[1], duetrade[0]);
                    // if(!response.data[i].send_date)
                    // {
                    //     htmlelements+='<td width="10%"></td>';
                    // }
                    // else if(formsenddte < duetradedte)
                    // {
