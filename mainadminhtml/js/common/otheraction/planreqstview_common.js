...
function getdataonload()
{
    ...
    website.ajax({
      url:'tradingplan/fetchplanforapprove',
      data:formdata,
      method:'POST',
      //contentType:'json',
             ...
             var planid = '';
              ...
              for(var i = 0; i < response.resdta.length; i++) 
              {
                ...
                //------------------------- Table Fields Insertion START ------------------------
                        ...
                        addhtmlnxt += '<td width="15%" class="trade"><i class="fa fa-eye faicon floatleft edittradeplan" title="Edit entry" id="edittrade" tradeplanid="'+response.resdta[i].id+'" uniquid="'+response.resdta[i].uniqueid+'"></i></td>'; 
                       addhtmlnxt += '</tr>';
...
website('body').on('click','.edittradeplan', function()
{
    ...
    window.location.href=baseHref+'tradingplan/planreqstapprv?tradeid='+trdeplnids+'&&uniqueid='+uniquid+ ''; 
});
