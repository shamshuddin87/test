...
{  var ckkval2='';
    ...
    website(".sendchkbox:checked").each(function(){
        selected_value.push(website(this).val());
        ckkval2 =website(this).attr('chkval2');
      //  alert(ckkval2);
    });
    
   if(selected_value!=''){
    website.ajax({
                  url:'exceptionreq/exceacceptapprovel',
                  data:{selctedid:selected_value,ckkval2:ckkval2},
                  method:'POST',
                  contentType:'application/x-www-form-urlencoded; charset=UTF-8',
                  dataType:"json",
                  cache:false,
...
var rejectid=website('#rejectapprov').attr('rejectid');
   ...
   website.ajax({
                  url:'exceptionreq/excerejectapprovel',
...
{
//    var itntfr = website(this).closest('.itntfr').attr('itntfr');
//    alert(itntfr);
    var rscrntpg = website(this).attr('p');
    //alert(rscrntpg);
    website('.personaldetails .panel.panel-white #pagenum').val(rscrntpg);    
    subuserapproval();    
});
...
website('body').on('click','.go_button', function(e) 
{
    ...
    var rscrntpg = website('.personaldetails .gotobtn').val();
    //alert(rscrntpg);
    website('.personaldetails .panel.panel-white #pagenum').val(rscrntpg);
    subuserapproval();
});
...
website('body').on("click",".viewexcprequsttrail",function(e){
