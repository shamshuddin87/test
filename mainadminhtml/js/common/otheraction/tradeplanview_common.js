             ...
             var sendstatus = [];
              ...
              for(var i = 0; i < response.resdta.length; i++) 
              {
                ...
                //------------------------- Table Fields Insertion START ------------------------
                  website('#reqstfr option[value='+response.resdta[i].requestfor+']').attr("selected","selected");
                  if(response.resdta[i].relative)
                  {
                  ...
                  website('#frmdate').val(fromdate);
                  website('#todate').val(todate);
                    
                    website('#cmpnme option[value='+response.resdta[i].companyid+']').attr("selected","selected");
                    ...
                    {
                       website('.sendtradeplan').show();
                        website('#edittradeplan').show();
                       addhtmlnxt += '<td width="15%"><i class="fa fa-edit faicon floatleft updatetrading" title="Edit entry" tradeplanid="'+response.resdta[i].id+'" uniquid="'+response.resdta[i].uniqueid+'"></i><i class="fa fa-trash-o faicon floatleft deletetrade" title="Delete entry" tradeplanid="'+response.resdta[i].id+'" ></i></td>'; 
                    }
                    if(response.resdta[i].send_status == 1 && response.resdta[i].approvedstatus == 0)
                    {
                        website('.sendtradeplan').hide();
                        website('#edittradeplan').hide();
...
//              if(jQuery.inArray('0', sendstatus) != -1)
//              {
//                  website('.sendtradeplan').show();  
//              }
//              else
...
//                  website('.sendtradeplan').hide();
//              }
              website('.sendtradeplan').attr('idss',planid);
              website('.appendtradeplanview').html(addhtmlnxt);
...
website('body').on('click','.sendtradeplan', function()
{
    var tradeid = website(this).attr('idss');
    var formdata = {tradeid:tradeid}
    website.ajax({
      url:'tradingplan/sendplanforapprv',
      data:formdata,
      method:'POST',
      //contentType:'json',
      contentType:'application/x-www-form-urlencoded; charset=UTF-8',
...
website('body').on('click','.updatetrading', function()
{
    var trdeplnids = website(this).attr('tradeplanid');
    var uniquid = website(this).attr('uniquid');
    var formdata ={trdeplnids:trdeplnids}
    website.ajax({
      url:'tradingplan/fetchtradeplanedit',
      data:formdata,
      method:'POST',
      //contentType:'json',
      contentType:'application/x-www-form-urlencoded; charset=UTF-8',
         ...
         if(response.logged === true)
          {
             ...
             var addhtmlnxt='';
              ...
              for(var i = 0; i < response.resdta.length; i++) 
              {
                ...
                var valueofsecu=response.resdta[i].valueofsecu?response.resdta[i].valueofsecu:'';
                var noofsecu=response.resdta[i].noofsecu?response.resdta[i].noofsecu:'';
                website('#requestfor').val(response.resdta[i].requestfor);
                website('#relative').val(response.resdta[i].relative);
                website('#companyid').val(response.resdta[i].companyid);
                website('#fromdate').val(fromdate);
                website('#todate').val(todate);
                website('#planid').val(response.resdta[i].id);
                //------------------------- Table Fields Insertion START ------------------------
                website('#sectype option[value='+response.resdta[i].secutype+']').attr("selected","selected");
                website('#datetype option[value='+response.resdta[i].datetype+']').attr("selected","selected");
                if(daterangefrm)
                {
                    website('#dtrngfrm').show();
                    website('#daterngfrm').val(daterangefrm);
                }
                if(daterangeto)
                {
                    website('#dterngto').show();
                    website('#daterngto').val(daterangeto);
                }
                if(specificdate)
                {
                    website('#specfdte').show();
                    website('#spficdate').val(specificdate);
                }
                  website('#noofsec').val(noofsecu);
                  website('#valueofsecurity').val(valueofsecu);
              }
...
website("#Mymodalplanedit #datetype").change(function() 
{
   var value = website(this).val();
   if(value == 1)
    {
        website('#Mymodalplanedit #specfdte').show();
        website('#Mymodalplanedit #dtrngfrm').hide();
        website('#Mymodalplanedit #dterngto').hide();
        website("#Mymodalplanedit #dtrngfrm").val('');
        website("#Mymodalplanedit #dterngto").val('');
        
    }
    else if(value == 2)
    {
        website('#Mymodalplanedit #specfdte').hide();
        website('#Mymodalplanedit #dtrngfrm').show();
        website('#Mymodalplanedit #dterngto').show();
        website("#Mymodalplanedit #specfdte").val('');
    }
    else
    {
        website('#Mymodalplanedit #specfdte').hide();
...
website('body').on('click','.deletetrade', function(){
    var id = website(this).attr('tradeplanid');
    //console.log(id);return false;
    website('#myModalyesno').modal('show');
    website('#myModalyesno .yesconfirm').attr('tradeplanid',id);
});
...
website('body').on('click','.yesconfirm', function(){
    ...
    website.ajax({
      ...
      url:'tradingplan/deletetradeplan',
      data:formdata,
      method:'POST',
      //contentType:'json',
      contentType:'application/x-www-form-urlencoded; charset=UTF-8',
