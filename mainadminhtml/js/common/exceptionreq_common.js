...
// var searchname=website("#nameofcmp").val();
...
function onkeysearchcmp(){ 
 ...
 if(search==''){
  website('#searchcmp').css("display", "none");
        ...
        website.ajax({
                  url:'exceptionreq/searchcompany',
                  data:formdata,
                  method:'POST',
...
var formdata = {approverid:approverid,reqname:reqname,typeofrequest:typeofrequest,selrelative:selrelative,sectype:sectype,idofcmp:idofcmp,nameofcmp:nameofcmp,noofshare:noofshare,typeoftrans:typeoftrans,sendreq:sendreq};
    ...
    website.ajax({
      url:'exceptionreq/exctradingrequests',
      data:formdata,
      method:'POST',
      //contentType:'json',
      contentType:'application/x-www-form-urlencoded; charset=UTF-8',
      ...
      success: function(response, textStatus, jqXHR)
      {
        if(response.logged === true)
         {
           website('.personaldetails #Mymodalreq').modal('hide');
             getalltradingrequest();
 ...
 website('#tradinformupdate').ajaxForm({
    ...
    success: function(response, textStatus, jqXHR) 
        ...
        if(response.logged === true)
     {
         ...
         website('.personaldetails #updatemodal').modal('hide');
         getalltradingrequest();
...
getalltradingrequest();
function getalltradingrequest()
{
    var noofrows = website('#noofrows').val(); 
    var pagenum = website('#pagenum').val();
    var myredirecturl=website('#redirecturl').val();
    var formdata = {noofrows:noofrows,pagenum:pagenum,redirecturl:myredirecturl};
  
     // alert(myredirecturl);
   website.ajax({
                  url:'exceptionreq/excgettradingrequest',
                  data:formdata,
                  method:'POST',
                  contentType:'application/x-www-form-urlencoded; charset=UTF-8',
                  dataType:"json",
                   ...
                   success: function(response, textStatus, jqXHR) 
                   {
                        if(response.logged==true)
                        {
                          ...
                          for(var i=0;i<response.data.length;i++)
                          {
                              ...
                              var message=response.data[i].rejmessage?response.data[i].rejmessage:'';
                              ...
                              var excep_approv=response.data[i].excepapp_status?response.data[i].excepapp_status:'';
                               //console.log(excep_approv);return false;
                                     ...
                                     addhtmlnxt += '<tr class="counter" tempid="'+response.data[i].id+'" >';
                                      //  if(send_status==0)
                                      // {
                                      //    addhtmlnxt += '<td width="15%"><input type="checkbox" class="sendchkbox" chkval="'+response.data[i].id+'" name="sendapprove" value="'+response.data[i].id+'"></td>';
                                      // }
                                      // else
                                      // {
                                      //     addhtmlnxt +='<td></td>';
...
website("body").on("click",".editper",function(e)
{
   ...
   website.ajax({
                  url:'exceptionreq/excgetsinglereq',
                  data:{editid:editid},
                  method:'POST',
                  contentType:'application/x-www-form-urlencoded; charset=UTF-8',
                  dataType:"json",
...
function onkeysearchcmpmodal(){ 
 ...
 if(search==''){
  website('#tradinformupdate #searchcmp').css("display", "none");
        ...
        website.ajax({
                  url:'exceptionreq/excsearchcompany',
                  data:formdata,
                  method:'POST',
                  contentType:'application/x-www-form-urlencoded; charset=UTF-8',
                  dataType:"json",
...
if(selected_value > 0){
    website.ajax({
                  url:'exceptionreq/sendmultiplereq',
...
website("body").on("click",".uploadfile",function(e){  
 ...
 var tradedate = website(this).attr('trading_date');
  website.ajax({
                  url:'exceptionreq/checktradestatus',
                  ...
                  { 
                   ...
                   success: function(response, textStatus, jqXHR) 
                   {
                         if(response.logged==true)
                         {
                             // console.log(response.data.demat_acc_no);
                               tradestatus=response.data['trading_status'];
                               var noofshare1=response.data.no_of_share?response.data.no_of_share:"";
                                // console.log(noofshare);
                               var priceofshare1=response.data.price_per_share?response.data.price_per_share:"";
                               var total1=response.data.total_amount?response.data.total_amount:"";
                               var transdate1=response.data.date_of_transaction?response.data.date_of_transaction:"";
                              // alert(transdate);
                                var myhtml='<tr><td><input type="text" name="noofshare" value="'+noofshare1+'" id="noofshare" class="form-control" size="20px"></td><td><input type="text" name="priceofshare" value="'+priceofshare1+'" id="pricepershare" class="form-control" size="20px">'+
                             '</td><td><input type="text" name="total" value="'+total1+'" id="totalamt" class="form-control" size="20px"></td><td><input type="text"  name="transdate" value="'+transdate1+'" class="form-control bootdatepick" size="20px"></td>';
                                var appendsel = '';
                              website.each(response.dematacc, function (index, value) {
                                  if(response.data.demat_acc_no==value['accountno'])
                                      {
                             ...
                             myhtml+='<td><select id="dmatacc" name="dmatacc" class="form_fields form-control col-md-7 col-xs-12" required>'+appendsel+'</select></td></tr>';
//                             website('#dmatacc select option[value="'+response.data.demat_acc_no+'"]').prop("selected","selected");
//                            
                              ...
                              website('.modtable').html(myhtml);
                              website('#uploadmyfile').modal('show');
                              datepicker();
                         }
                         else{
                              var myhtml='<tr><td><input type="text" name="noofshare" value="'+noofshare+'" id="noofshare" class="form-control" size="20px"></td><td><input type="text" name="priceofshare" value="'+priceofshare+'" id="pricepershare" class="form-control" size="20px">'+
                             '</td><td><input type="text" name="total" value="'+total+'" id="totalamt" class="form-control" size="20px"></td><td><input type="text"  name="transdate" value="'+transdate+'" class="form-control bootdatepick" size="20px"></td>'; 
                             var appendsel = '';
                              website.each(response.dematacc, function (index, value) {  
                               appendsel += '<option value='+value['accountno']+'>'+value['accountno']+'</option>';
...
website("body").on("click",".notrade",function(e){ 
   website(".dispalytrade").css("display", "none");
   website(".notrading").css("display", "block");
});  
website("body").on("click",".yestrade",function(e){ 
   website(".dispalytrade").css("display", "block");
   website(".notrading").css("display", "none");
});  
...
var reqid=website(this).attr('reqid');
 website.ajax({
                  url:'exceptionreq/getsuccesstrade',
                  data:{reqid:reqid},
                  method:'POST',
                  contentType:'application/x-www-form-urlencoded; charset=UTF-8',
                  dataType:"json",
                  ...
                  { 
                   ...
                   {
                      if(response.logged==true){
                       var noofshare=response.data.no_of_share?response.data.no_of_share:'Not Found';
                       var price_per_share=response.data.price_per_share?response.data.price_per_share:'Not Found';
                       var total_amount=response.data.total_amount?response.data.total_amount:'Not Found';
                       var date_of_transaction=response.data.date_of_transaction?response.data.date_of_transaction:'Not Found';
                       var demat_acc_no=response.data.demat_acc_no?response.data.demat_acc_no:'Not Found';
                       var addhtmlnxt ='<tr>';
...
website('body').on("click",".excprequsttrail",function(e){
    var rqstid = website(this).attr('rqstid');
    var trdeid = website(this).attr('trdeid');
    var formdata = {rqstid:rqstid,trdeid:trdeid}
    website.ajax({
      url:'exceptionreq/fetchexcereqtrail',
      data:formdata,
      method:'POST',
      //contentType:'json',
      contentType:'application/x-www-form-urlencoded; charset=UTF-8',
