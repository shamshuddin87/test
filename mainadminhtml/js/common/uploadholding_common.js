...
website('#insertholding').ajaxForm({
    //method:'POST',
    //contentType:'json',
...
function getdataonload()
{
    ...
    website.ajax({
        url:'uploadholding/fetchholding',
        data:formdata,
        method:'POST',
        //contentType:'json',
        contentType:'application/x-www-form-urlencoded; charset=UTF-8',
        ...
        success: function(response, textStatus, jqXHR)
        {
            if(response.logged===true)
            {
                ...
                for(var i = 0; i < response.resdta.length; i++) 
                {
                    ...
                    var dateofholding = response.resdta[i].dateofholding?response.resdta[i].dateofholding:'';
                    addhtmlnxt += '<tr class="counter" holdingid="'+response.resdta[i].id+'">';
                    addhtmlnxt += '<td width="25%">'+dateofholding+'</td>';
                    addhtmlnxt += '<td width="15%">'+response.resdta[i].type_of_holding+'</td>';
                    addhtmlnxt += '<td width="25%">'+ddmmyy+'</td>';
                    addhtmlnxt += '<td width="25%">'+times+'</td>';

                    addhtmlnxt += '<td width="30%"><i class="fa fa-eye faicon floatleft viewholding" title="View entry" id="edittrade" reconciid="'+response.resdta[i].id+'" uniqueid="'+response.resdta[i].uniqueid+'" tilldate="'+response.resdta[i].dateofholding+'"></i></td>';  
...
website('body').on('click','.viewholding', function()
{
    ...
    window.location.href=baseHref+'uploadholding/viewholding?reconciid='+reconciid+'&&uniqueid='+uniquid+ '&&dateofrecon='+dateofrecon+''; 
});
